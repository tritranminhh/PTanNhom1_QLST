...
public class GUIBanHang extends JFrame{
	...
	private String[] itemsTB = {
			"Tên sản phẩm", "Loại sản phẩm", "Màu Sắc", "Nhà cung cấp", "Xuất sứ",
			"Hạn bảo trì", "Đơn vị tính", "Số lượng", "Giá trên mỗi sản phẩm", "Mô tả"
	};
	//Hóa đơn
	private JComboBox<String> jbHD;
	private String[] itemsHD = {
			"STT", "Tên sản phẩm", "Loại Sản phẩm", "Đơn vị tính", "Số lượng", "Giá trên mỗi sản phẩm", "Thành tiền"
	};
	//DS hóa đơn
	...
	//QLTB
	private QLSanPhamimpl dsTB = new QLSanPham(em);
	//QLHD
	private QLHoaDonimpl dsHD = new QLHoaDon(em);
	...
	private QLKhachHangimpl dsKH = new QLKhachHang(em);
	/**
	 * dinh dang
	 */
	...
	public GUIBanHang(NhanVien Activity) {
		...
		jTabbedPane.addTab("Danh sách sản phẩm", bDSTB);
		//table
	...
	public void updateTableTBData() {
		...
		for(SanPham x : dsTB.getDSTB()) {
			dfmodelTB.addRow(new Object[] {
				x.getMaTB(), x.getLoaiTB().getTenLTB(), x.getMauSac().getTenMau(), 
	...
	public void updateTableHDData() {
		if(HD.getMaHD() != null) {
			...
			for(int index = 0; index < HD.getDsSP().size(); index++) {
				SanPham x = HD.getDsSP().get(index);
	...
	public void btThemGioHangActionPerformed(ActionEvent evt){
		int rowSelected = tableTB.getSelectedRow();
		if(rowSelected != -1) {
			SanPham x  = dsTB.SearchTB(tableTB.getValueAt(rowSelected, 0).toString());
			String sl = JOptionPane.showInputDialog("Nhập số lượng sản phẩm cần mua:");
			System.out.println(tableTB.getValueAt(rowSelected, 0).toString());
			try {
				
				int soLuong = Integer.parseInt(sl);
				if(soLuong < 0) {
					JOptionPane.showMessageDialog(null, "Số lượng sản phẩm không được bé hơn 0!!", "Thông báo:", 1);
				} else if(soLuong > x.getSoLuong()) {
					
					JOptionPane.showMessageDialog(null, "Số lượng sản phẩm không được lớn hơn\nsố lượng thiết bị trong kho!!", "Thông báo:", 1);
					...
					if(dsTBN.contains(x)) {
						...
						List<Integer> dsSLMua = HD.getSoLuong();
						if(dsSLMua == null)
	...
	public void btSuaGioHangActionPerformed(ActionEvent evt) {
		...
		if(rowSelected != -1 && !dsHD.getDSHD().contains(HD)) {
			...
			try {
				int old = HD.getSoLuong().get(rowSelected);
				int soLuong = Integer.parseInt(sl);
				SanPham x = HD.getDsSP().get(rowSelected);
				if(soLuong < 0) {
